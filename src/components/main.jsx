            ...
            <div>
                {anime.length === 0 ? (
                    ...
                    <div className="grid lg:grid-cols-3 sm:grid-cols-1 gap-5 p-6">
                        {anime.map((anime) => (
                            <div key={anime.id} className="card hover:scale-95 transition-all m-2 overflow-hidden bg-gray-200  rounded-lg shadow-lg relative">
                                <img src={anime.images.jpg.image_url} className='h-full w-full rounded-md ' alt="" />
                                <div className="absolute bg-zinc-800 bg-opacity-75 text-white top-0 left-0 w-full h-full flex flex-col justify-center items-center opacity-0 hover:opacity-100 hover:text-2xl transition-all">
